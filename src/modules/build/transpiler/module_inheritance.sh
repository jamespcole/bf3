...
build.transpiler.init() {
    ...
    build.transpiler.transpile() {
        ...
        varsArrCode="${varsArrCode}@this[namespace]='$nSpace'\n\n"
        wrapped="${varsArrCode}\n${wrapped}\n"
        ...
        transpiled=$(echo -e "${wrapped}" \
            | sed "s/@this\.super\./${nSpaceAlias}.super./g" \
            | sed "s/@this\./${nSpace}./g" \
            | sed "s/@this:/${nSpace}:/g" \
            | sed "s/@params\[\([^]]*\)\]=/params['\1']=/g" \
            | sed "s/@params\[\([^]]*\)\]/\${params['\1']}/g" \
            | sed "s/@=>params/\"\${FUNCNAME[0]}::args\"; local -A params; parameters.load --namespace \"${nSpace}.\${FUNCNAME[0]##*.}\" --args \"\$@\"\n/" \
        ...
        while read functionName; do
            if [ "${operation}" == 'extend' ] && [ "${functionName}" != "${nSpaceOuter}.init" ] && [[ $functionName != __global::* ]]; then
                ...
                # local superCall="${nSpaceAlias}._super.${originalNamespace}.${functionName}"
                # extraFunctions="${extraFunctions}\n    ${baseNamespace}.${functionName}(){"
                # extraFunctions="${extraFunctions}\n        ${baseNamespace}.super.${functionName} \"\$@\""
                # extraFunctions="${extraFunctions}\n    }\n"
                ...
                # extraFunctions="${extraFunctions}\n    ${nSpaceAlias}._super.${functionName}(){"
                # extraFunctions="${extraFunctions}\n        ${superCall} \"\$@\""
                # extraFunctions="${extraFunctions}\n    }\n"
                ...
                # echo "${nSpaceAlias//._super/ }"
                local baseNs
                local fullNs="${nSpaceAlias}._super.${originalNamespace}"
                build.transpiler.logToErr "Getting heirarchy ${fullNs}"
                local lines="${fullNs//._super/\\n}"
                local rootNs=''
                local prevNs=''
                local prefix="${baseNamespace}"
                local prevPrefix="${baseNamespace}"
                local inheritedFuncs=''
                while read -r baseNs; do
                    ...
                    if [ "${rootNs}" == '' ]; then
                        ...
                        baseNs="${baseNs#*.}"
                        prefix="${prefix}.super.${baseNs}"
                        # if [ "${prevNs}" == "$rootNs" ]; then
                        #     extraFunctions="${extraFunctions}\n    ${baseNamespace}.super.${functionName}(){"
                        #     extraFunctions="${extraFunctions}\n       ${prefix//.super/._super}.${functionName} \"\$@\""
                        #     extraFunctions="${extraFunctions}\n    }\n"
                        #
                        # else
                        #
                        #     extraFunctions="${extraFunctions}\n    ${prevPrefix}.super.${functionName}(){"
                        #     extraFunctions="${extraFunctions}\n        ${prefix}.${functionName} \"\$@\""
                        #     extraFunctions="${extraFunctions}\n    }\n"
                        # fi
                        inheritedFuncs="${inheritedFuncs}\n${prefix//.super/._super}.${functionName}"
                    ...
                    build.transpiler.logToErr "------ ${baseNs}"
                    #
                    # extraFunctions="${extraFunctions}\n    ${baseNamespace}.${baseNs}.super.${functionName}(){"
                    # extraFunctions="${extraFunctions}\n        ${superCall} \"\$@\""
                    # extraFunctions="${extraFunctions}\n    }\n"

                done <<< "$(echo -e $lines)"

                build.transpiler.logToErr "${inheritedFuncs}"
        ...
        local nsPrefix="${nSpace}"
        if [ "${operation}" == 'extend' ]; then
            nsPrefix="${nSpaceAlias}._super.${originalNamespace}"
        fi
    ...
    build.transpiler.getOuterNamespace(){
        ...
        [ "${importOp}" == 'extend' ] && {
            ...
            nameSpaceOuter="${nSpaceAlias}._super.${originalNamespace}"
        }
        echo "${nameSpaceOuter}"
    }
}
